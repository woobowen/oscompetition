...
#define N_SOCKET 32
#define SOCK_STREAM_BUF 4096
#define SOCK_UDP_QUEUE 8
#define SOCK_UDP_DATA 1536
...
static socket_t *find_bound_locked(int type, uint16 port)
{
    for (int i = 0; i < N_SOCKET; i++) {
        socket_t *so = &sockets[i];
        if (!so->used || !so->bound)
...
int socket_bind(socket_t *so, uint64 user_addr, uint32 addrlen)
{
    ...
    if (port == 0 || port_in_use_locked(so->type, port)) {
        spinlock_release(&socket_lk);
        return -EADDRINUSE;
    }
    so->bound = 1;
...
int socket_sendto(socket_t *so, uint64 user_buf, uint32 len, int flags, uint64 user_addr, uint32 addrlen)
{
    ...
    if (type == SOCK_DGRAM_LOCAL) {
        ...
        if (len > SOCK_UDP_DATA)
            return -EMSGSIZE;
        if (len > 0)
            uvm_copyin(myproc()->pgtbl, (uint64)tmp, user_buf, len);
        ...
        socket_t *dst = find_bound_locked(SOCK_DGRAM_LOCAL, port);
        if (dst == NULL) {
            spinlock_release(&socket_lk);
            return -ECONNREFUSED;
        }
        while (dst->udp_count >= SOCK_UDP_QUEUE) {
            if (so->nonblock) {
                spinlock_release(&socket_lk);
                return -EAGAIN;
            }
...
int socket_getsockopt(socket_t *so, int level, int optname, uint64 optval, uint64 optlen)
{
    ...
    if (level == SOL_SOCKET_LOCAL && (optname == SO_SNDBUF_LOCAL || optname == SO_RCVBUF_LOCAL))
        ...
        value = 0;
    else if (level == SOL_SOCKET_LOCAL && (optname == SO_REUSEADDR_LOCAL || optname == SO_KEEPALIVE_LOCAL))
        value = 0;
    else
        value = 0;
...
int socket_poll_ready(socket_t *so, int events)
{
    ...
    if (events & POLLIN_LOCAL) {
        if (so->state == SOCK_LISTEN && so->accept_count > 0)
