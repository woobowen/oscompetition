...
def prepare_sdcard(usb_port):
    ...
    outf = open(image_sdcard_write_output_path, "w+", encoding='utf-8')
...
def ssh_run(usb_port):
    ...
    flash_cmd = f"kflash -b 1000000 -B dan /cg/{port_dir}/img1.kfpkg -p {usb_port} > /cg/{port_dir}/os_flash_out.txt"
    print(flash_cmd, file=sys.stderr)
    # with open(f"/cg/{port_dir}/os_flash_out.txt", "w") as f:
    #     f.write(flash_cmd + '\n')
    # print(flash_cmd)
    ...
    for i in range(3):
        process = subprocess.Popen(flash_cmd, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                   encoding="utf8")
        out, err = process.communicate()
        with open(f"/cg/{port_dir}/os_flash_out.txt") as f:
            data = f.read()
            if "[ERROR]" in data:
                if i == 2:
    ...
    #loge("\n[os autotest]: Rebooting\n")

    # # read from serial
    #loge("\n[os autotest]: Read from serial\n")
